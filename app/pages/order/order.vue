<template>
	<view class="content">
		...
		<view class="item" v-for="item in waiguan" :key="item.value" @tap="">
			<view class="">
				{{item.label}}：
			</view>
			<image class="vin_url" v-if="record.vin_url!=''" :src="record.vin_url" mode="aspectFit" @tap=""></image>
		</view>
		...
		<uni-popup ref="signature" type="bottom" :custom="true">
			<view class="popup-body signature">
				...
				<view>
					<canvas :style="{width: sign_plate.width, height: sign_plate.height}" canvas-id="firstCanvas" class="signature-body"
...
<script>
	...
	export default {
		...
		onLoad() {
			this.getWaiguanItems()
		},
		methods: {
			getWaiguanItems() {
				var _this = this
				helper.ajax.get('/v1/dict/waiguan', null, res => {
					console.log(res)
					if (res.status === 200) {
