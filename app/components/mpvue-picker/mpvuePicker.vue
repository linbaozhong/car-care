<template>
  <view class="mpvue-picker">
    ...
    <view class="mpvue-picker-content " :class="{'mpvue-picker-view-show':showPicker}">
      ...
      <!-- 年月选择器 -->
      <picker-view indicator-style="height: 40px;" class="mpvue-picker-view" :value="pickerValue" @change="pickerDateChange" v-if="mode==='yearMonthSelector'">
        <block>
          <picker-view-column>
         ...
         <picker-view-column v-if="mode!=='yearMonthSelector'">
      ...
      <picker-view indicator-style="height: 40px;" class="mpvue-picker-view" :value="pickerValue" @change="pickerChange" v-if="mode==='multiSelector'">
        <block v-for="(column,index) in pickerValueMulArray" :key="index">
          <picker-view-column>
            <view class="picker-item" v-for="(item,index1) in column" :key="index1">{{item.label}}</view>
          </picker-view-column>
        </block>
      </picker-view>
      <!-- 二级联动 -->
...
export default {
  ...
  methods: {
    initPicker(valueArray) {
      ...
      } else if (this.mode === 'dateSelector' || this.mode === 'yearMonthSelector') {
        let year, month, day;
        // 支持传入 Date 对象
    ...
    _getPickerLabelAndValue(value, mode) {
      ...
      } else if (mode === 'dateSelector' || mode === 'yearMonthSelector') {
        pickerLable =
